...
// A 24 byte string class (can be as small as 17 for x64 and 13 for x86) that can hold strings with
// length of up to 23 chars on the stack before going on the heap - the last byte of the buffer is
// used for:
...
template <typename T>
struct has_insertion_operator<T, decltype(operator<<(declval<std::ostream&>(), declval<const T&>()),
...
template <typename T, typename detail::types::enable_if<
                          detail::should_stringify_as_underlying_type<T>::value, bool>::type = true>
String toString(const DOCTEST_REF_WRAP(T) value) {
...
#define DOCTEST_DO_BINARY_EXPRESSION_COMPARISON(op, op_str, op_macro)                           \
    ...
    DOCTEST_NOINLINE SFINAE_OP(Result, op) operator op(R&& rhs) {                               \
        bool res =                                                                              \
            op_macro(doctest::detail::forward<const L>(lhs), doctest::detail::forward<R>(rhs)); \
...
struct DOCTEST_INTERFACE IReporter {
    ...
    // called when a test case is reentered because of unfinished subcases (safe to cache a pointer
    // to the input)
    ...
    // called when a test case is skipped either because it doesn't pass the filters, has a skip
    // decorator or isn't in the execution range (between first and last) (safe to cache a pointer
    // to the input)
    ...
    // can iterate through contexts which have been stringified automatically in their destructors
...
#define DOCTEST_BINARY_ASSERT(assert_type, comp, ...)                                             \
    DOCTEST_FUNC_SCOPE_BEGIN {                                                                    \
        doctest::detail::ResultBuilder DOCTEST_RB(doctest::assertType::assert_type, __FILE__,     \
                                                  __LINE__, #__VA_ARGS__);                        \
        DOCTEST_WRAP_IN_TRY(                                                                      \
            DOCTEST_RB.binary_assert<doctest::detail::binaryAssertComparison::comp>(__VA_ARGS__)) \
        DOCTEST_ASSERT_LOG_REACT_RETURN(DOCTEST_RB);                                              \
    }                                                                                             \
    DOCTEST_FUNC_SCOPE_END
...
#define DOCTEST_EXCEPTION_EMPTY_FUNC                                                          \
    [] {                                                                                      \
        static_assert(false,                                                                  \
                      ...
                      "Use DOCTEST_CONFIG_NO_EXCEPTIONS_BUT_WITH_ALL_ASSERTS if you want to " \
...
#define DOCTEST_INTERNAL_ERROR(msg) \
    throw_exception(                \
        std::logic_error(__FILE__ ":" DOCTEST_TOSTR(__LINE__) ": Internal doctest error: " msg))
        ...
        if (g_cs->subcaseStack[g_cs->currentSubcaseDepth] == m_signature) {
            ...
            DOCTEST_ITERATE_THROUGH_REPORTERS(subcase_start, m_signature);
                   ...
                   g_cs->fullyTraversedSubcases.find(hash(
                       hash(g_cs->subcaseStack, g_cs->currentSubcaseDepth), hash(m_signature))) ==
...
struct FatalConditionHandler {
    ...
    FatalConditionHandler() {
        ...
        std::set_terminate([]() DOCTEST_NOEXCEPT {
            ...
            if (isDebuggerActive() && !g_cs->no_breaks) DOCTEST_BREAK_INTO_DEBUGGER();
            std::exit(EXIT_FAILURE);  // explicitly exit - otherwise the SIGABRT handler may be
                                      // called as well
        });
...
int Context::run() {
    ...
    // save the old context state in case such was setup - for using asserts out of a testing
    // context
    auto old_cs = g_cs;
